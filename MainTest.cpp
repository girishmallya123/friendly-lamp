...
public:
    ...
    virtual bool IsFirstAboveSecondBackwards(string _first, string _second);
};
...
bool AlphabeticalAscendingStringComparer::IsFirstAboveSecondBackwards(string _first, string _second) {
	unsigned int i=_first.size()-1;
    unsigned int j=_second.size()-1;
	...
	for (unsigned int i = 0; i < sortedList.size() - 1; ++i) {
		for (unsigned int j = 0; j < sortedList.size() - 1; ++j) {
            ...
            if(_sortType == ESortType::AlphabeticalAscending) {
                ...
                if (stringSorter->IsFirstAboveSecondBackwards(sortedList[j], sortedList[j+1])) {
                    string tempString = sortedList[j];
                    sortedList[j] = sortedList[j+1];
                    sortedList[j+1] = tempString;
                }
